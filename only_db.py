...
if __name__ == '__main__':
    ...
    while a != '0':
        if a == '1':
            ...
            data1 = db.get_place(snils)
            data2 = db.get_place(snils, False)

            print('\nЕсли все перед тобой подадут оригинал, ты поступишь на: ')
            if data1:
                for data in data1:
                    print(f'{data[0]} - {data[1]} {"(договор)" if data[2] else "(бюджет)"}\nТы - {data[3]} по счёту, '
            ...
            print('Судя по текущим данным о поданных оригиналах, ты поступишь на: ')
            if data2:
                data2 = data2[0]
                print(f'{data2[0]} - {data2[1]} {"(договор)" if data2[2] else "(бюджет)"}\nТы - {data2[3]} по счёту, '
                      f'{data2[4]}, по подавшим оригинал, ещё {data2[5]} человек перед тобой могут подать оригинал.\n')

            print('*Здесь может быть максимум 1 направление из ВУЗа с аттестатом.\n'
            ...
            print('\nПроходные на бюджетные направления:')
            print(*db.get_prohodnoy(data), sep='\n')
            print('\nПроходные на платные направления:')
            print(*db.get_prohodnoy(data, 1), sep='\n')
            print()
